...
# 1. Finds Pets by status (пример из теории)
status = 'available'
...
if 'application/json' in res_get_by_status.headers['Content-Type']:  # Эта структура - из юнита 19.3
    print(f'Тело ответа:')
    print(res_get_by_status.json())  # Здесь для построчного отображения хотелось стянуть форматирование из готового
    # репозитория, но тогда получалось слишком много строк, поэтому оставила, как есть
else:
    print(f'Тело ответа:')
...
# 5. Uploads an image - честно списала
photo = 'doge.jpeg'
...
orderId = 2  # По Swagger д. б. значение от 1 до 10, но, видимо, может выдавать 404, если кто-то удалит по этому ID.
...
new_orderId = res_post_order.json()['id']  # Здесь - положительное целочисленное, поэтому удалим то, что создавали выше
